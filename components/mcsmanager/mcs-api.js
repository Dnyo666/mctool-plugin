...
export default class McsAPI {
  ...
  async getOverview(qq) {
    ...
    try {
      ...
      // 检查并记录守护进程信息
      if (!data.daemon || data.daemon.length === 0) {
        throw new Error('未找到可用的守护进程');
      }

      logger.debug(`[MCS API] 找到守护进程:`, data.daemon.map(d => ({
        name: d.name,
        uuid: d.uuid
      })));

      return {
        daemon: data.daemon.map(d => ({
          uuid: d.uuid,
          name: d.name,
          ip: d.ip,
          port: d.port,
  ...
  async sendCommand(qq, instanceUuid, command) {
    await this.initUserConfig(qq);
    try {
      const url = this.buildUrl(`/api/protected_instance/command`);
      const response = await fetch(url, {
        method: 'POST',
        headers: {
          ...this.headers,
   ...
   */
  async getInstanceLog(qq, instanceUuid) {
    await this.initUserConfig(qq);
    try {
      const url = this.buildUrl(`/api/protected_instance/outputlog?uuid=${instanceUuid}`);
      const response = await fetch(url, {
        method: 'GET',
        headers: this.headers
      });
   ...
   * 批量实例操作
   * @param {string} qq QQ号
   * @param {string[]} instanceUuids 实例UUID数组
   * @param {string} operation 操作类型(open/stop/restart/kill)
   * @returns {Promise<Object>} 操作结果
   */
  async batchInstanceOperation(qq, instanceUuids, operation) {
    await this.initUserConfig(qq);
    try {
      const url = this.buildUrl(`/api/instance/multi_${operation}`);
      const response = await fetch(url, {
        method: 'POST',
        headers: {
          ...this.headers,
        ...
        body: JSON.stringify(instanceUuids)
      });

      const data = await this.handleResponse(response);
      return data;
    } catch (error) {
      logger.error(`[MCS API] 批量${operation}操作失败:`, error);
      throw error;
    }
  }
