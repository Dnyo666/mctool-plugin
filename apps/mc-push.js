    ...
    try {
        ...
        for (const [groupId, groupConfig] of Object.entries(subscriptions)) {
            ...
            if (oldStatus?.online !== newStatus?.online) {
                ...
                logger.debug(`[MCTool] 服务器 ${serverName} ${newStatus.online ? '上线' : '离线'} 推送至群 ${groupId}`);
            }
...
function handlePlayerChanges(serverId, serverName, groupId, oldStatus, newStatus) {
    try {
        ...
        for (const player of joined) {
            ...
            logger.debug(`[MCTool] 玩家 ${player} ${isNew ? '首次加入' : '加入'} ${serverName} 推送至群 ${groupId}`);
        }
        ...
        for (const player of left) {
            ...
            logger.debug(`[MCTool] 玩家 ${player} 离开 ${serverName} 推送至群 ${groupId}`);
        }
...
function updateHistoricalPlayers(serverId, players) {
    try {
        ...
        if (newPlayers.length > 0) {
            ...
            logger.debug(`[MCTool] 服务器 ${serverId} 新增历史玩家: ${newPlayers.join(', ')}`);
        }
