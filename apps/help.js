...
export class Help extends plugin {
    ...
    async mcmodHelp(e) {
        try {
            // 使用puppeteer渲染HTML
            const htmlPath = path.join(process.cwd(), 'plugins', 'mctool-plugin', 'resources', 'html', 'mod-help.html');
            const browser = await puppeteer.browserInit();
            const page = await browser.newPage();
            ...
            // 截图
            const screenshotPath = path.join(tempDir, `${e.user_id}_mod_help.png`);
            try {
                await page.screenshot({
                    path: screenshotPath,
                    fullPage: true
            ...
            console.error('生成Mod帮助信息失败:', error);
            e.reply('生成Mod帮助信息失败，请稍后重试');
            return false;
        }
    }
}
